...
**The Failure:** When the system hit the token limit and fell back to the smaller 8B model, the 8B model failed completely. Small LLMs suffer heavily from "Negative Constraint Confusion"—they struggle to understand what *not* to do and end up ignoring the bans.

### Phase 3: Aggressive Positive Directives & Formatting
To fix the 8B model, we overhauled the `RESPONSE RULES`. Instead of just telling it what *not* to do, we gave it hyper-specific instructions on *how* to act:
...
2. **Strict Markdown Formatting:** The 8B model previously clumped bullet points together. We fixed this by mandating: *"ALWAYS put each bullet on its own separate NEW LINE — never run bullets together."*
...
By leveraging positive directives, the 8B model is completely unchained. Even when the 70B token quota runs dry, the portfolio provides sharp, heavily formatted, highly technical answers that feel just as intelligent.
