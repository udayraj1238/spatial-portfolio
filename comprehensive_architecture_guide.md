...
2. **The Intelligence Engine:** A Vercel Edge Runtime backend connecting to Groq's ultra-fast Llama 3 LLMs, bound by aggressive rate-limiting, dynamic token budgeting, and advanced prompt engineering.
...
### B. Markdown Parsing & The `remark-gfm` Experiment
**The Experiment:** LLMs naturally output markdown. We initially used standard `ReactMarkdown` to render the text. However, we found that when the AI output raw URLs (e.g., `github.com/udayraj1238`), they were rendered as unclickable raw text.
**The Solution:** We integrated `remark-gfm` (GitHub Flavored Markdown). This plugin intercepts the AST (Abstract Syntax Tree) of the markdown and automatically detects raw URLs, wrapping them in `<a>` tags. Furthermore, we passed a custom `components` prop to `ReactMarkdown` to force all `<a>` tags to have `target="_blank"`, ensuring users don't accidentally navigate away from the portfolio when clicking a project link.
...
**The Experiment:** Groq's API restricts the highly intelligent `llama-3.3-70b-versatile` model to exactly 100,000 Tokens Per Day (TPD). During our first day of testing, we hit this limit in a single afternoon, crashing the app.
**The Solution:** We implemented a `dailyTokensUsed` counter. Every request estimates token usage (`systemTokens + historyTokens + 400`). 
Once `dailyTokensUsed` exceeds 80,000 (leaving 20k for safety), the `shouldUseFallback()` function returns true. The API seamlessly hot-swaps the model to `llama-3.1-8b-instant`, which has a massive 1,000,000 TPD limit. The user experiences zero downtime.
...
### D. The Vercel AI SDK MaxTokens Bug (Bug 3)
**The Experiment:** We initially passed `maxTokens: 600` to the `streamText` function to prevent the AI from generating endlessly long essays. 
**The Failure:** The Next.js build failed with a severe TypeScript error: `maxTokens does not exist in type CallSettings`. It turns out the specific Groq provider package for the Vercel AI SDK had a typing conflict regarding the `maxTokens` parameter.
**The Solution:** Rather than fighting the types or using `@ts-ignore` (which is bad practice), we completely removed the `maxTokens` property. We shifted the burden to Prompt Engineering, explicitly instructing the AI to limit its word count.
...
### Phase 2: Negative Constraints (The 8B Fallback Failure)
When the AI answered questions, it often sounded like a cheesy HR recruiter ("Uday is a plethora of skills and a valuable asset"). 
To fix this, we added negative constraints: `BANNED WORDS: plethora, testament, valuable asset.`
**The Failure:** When the system fell back to the smaller 8B model (due to the token limit), the 8B model completely broke down. Small LLMs suffer heavily from "Negative Constraint Confusion"—if you tell them "Don't think of a pink elephant," they will output a pink elephant. The 8B model ignored the banned words and continued outputting generic fluff.

### Phase 3: Aggressive Positive Directives & Formatting
To fix the 8B model, we overhauled the `RESPONSE RULES`. Instead of telling it what *not* to do, we gave it hyper-specific instructions on *how* to act:
1. **Argumentation Synthesis:** We added: *"If asked for an opinion, DO NOT just blindly list grades. Synthesize a direct, highly convincing argument... use hardcore engineering feats as proof."* This forces the LLM to actively reason rather than summarize.
2. **Strict Markdown Formatting:** The AI kept outputting bullet points on a single line, causing the markdown parser to render giant walls of text. We added: *"BULLETS: ALWAYS put each bullet point on a NEW LINE."*
3. **Strict Link Syntax:** To support `remark-gfm`, we added: *"LINKS: ALWAYS use proper Markdown link syntax: '[Text](https://...)'. NEVER output raw URLs."*

By shifting to positive directives, both the 70B and 8B models now generate sharp, heavily formatted, highly technical answers that are extremely scannable for recruiters.
...
Every bug faced—from WebGL context collisions to LLM negative constraint failures—was met with an engineered, robust solution that prioritizes performance, cost-efficiency, and user experience above all else.
